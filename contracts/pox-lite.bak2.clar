...
(define-public (append-deposit (amount uint) (memo (buff 70)) (height uint))
  (begin
    (var-set high (+ (var-get high) amount))
    (match (get-deposits-by-height height)
      deps (if (map-set deposits { block-height: height}
                 (unwrap! (as-max-len? (append deps { address: tx-sender, amount: amount, high: (var-get high), memo: memo }) u100)
                   (err u2)))
                 (ok true)
                 (err u3))
      error (err error)
...
(define-public (deposit (amount uint) (memo (buff 70)) (height uint))
  (begin
    ...
    (match (stx-transfer? amount tx-sender (as-contract tx-sender))
      deps (if (map-insert deposits { block-height: height } (list { address: tx-sender, amount: amount, high: (var-get high), memo: memo }))
        (ok true)
...
;;  (begin
    (deposit amount memo block-height)
